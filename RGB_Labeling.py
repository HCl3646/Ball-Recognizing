...
while True:
    ...
    for i in range(len(stats)):
        ...
        if (h, w) < dst.shape and 500 * 4 < w * h < 3000 * 4 and 2 / 3 < w / h < 4 / 3 and 0.8 < area / (
            ...
            cv2.line(src, (int(mx), int(my)),
                     (int(mx), int(my)), (0, 0, 255), 3)
            point = (mx, my, idx)
            pts.append(point)
...
for i in balls:
    for j in i:
        (write_ws.cell(column=4*balls.index(i) +
         1, row=i.index(j)+3)).value = j[0]
        (write_ws.cell(column=4*balls.index(i) +
         2, row=i.index(j)+3)).value = j[1]
        (write_ws.cell(column=4*balls.index(i) +
         3, row=i.index(j)+3)).value = j[2]
